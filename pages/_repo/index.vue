<template>
  <div v-hotkey="keymap" class="bg-white lg:min-w-0 lg:flex-1">
    ...
    >
      <div class="flex space-x-4 items-center bg-white">
        <h1 class="flex-init w-1/4 text-lg font-medium">
          {{ data.repo }} Tests
        </h1>
        <div class="flex-1 w-1/3 flex space-x-4">
          <VueAutosuggest
          ...
          <button
            ...
            :class="[
              ...
              'inline-flex items-center content-center p-1 border border-transparent rounded-2xl shadow-sm text-gray-600 w-10 hover:bg-blue-100 focus:outline-none',
            ]"
            @click="toggleFilter"
          >
        ...
        <div class="flex w-1/3 content-end text-right">
          <div
            v-if="data.tests"
            class="flex-init relative col-span-1 flex shadow-sm rounded-md"
          >
            <div
              :class="[
                'bg-green-500',
                'font-bold flex-shrink-0 flex items-center justify-center w-16 text-white text-sm font-medium rounded-l-md',
              ]"
            >
              {{ data.tests.length }}
            </div>
...
export default {
  ...
  asyncData({ $axios, params }) {
    return (
      $axios
        .get(`${$axios.defaults.baseURL}/data/${params.repo}.json`)
